...
model = SAC('MultiInputPolicy', env, replay_buffer_class=HerReplayBuffer, ent_coef='auto',
            ...
            gamma=0.95, batch_size=64, tensorboard_log="./sac_pickplace/",
            policy_kwargs=dict(net_arch=[64, 64]))

model.learn(total_timesteps=100) 

model.save(str(currpath)+"/sac_pickplace")
model.save_replay_buffer(str(currpath)+"/sac_replay_buffer")

loaded_model = SAC.load("sac_pickplace.zip", env=env)
